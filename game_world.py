...
class GameWorld:
    ...
    def fire_bullet(self):
        if self.agent.can_fire():
            bullet = Bullet(self.agent.body.position, self.agent.body.angle, self.space)
            self.bullets.append(bullet)
            self.entities.append(bullet)  
            self.agent.ammo -= 1
            self.agent.last_fire_time = time.time()
